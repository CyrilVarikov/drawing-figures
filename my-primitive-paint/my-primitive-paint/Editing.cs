    ...
    public class Editing
    {
        ...
        public MainFigure PickFigure(List<MainFigure> figures, Point location)
        {
            foreach(var figure in figures)
            {
                ...
                {
                    var croch = (MainFigure)Activator.CreateInstance(figure.GetType(), 4, Color.CornflowerBlue, new Point(figure.topLeft.X - 4, figure.topLeft.Y - 4), new Point(figure.bottomRight.X + 4, figure.bottomRight.Y + 4));                   
                    return croch;
                } 
            }
            return null;
